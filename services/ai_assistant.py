...
def _get_hf_settings(app):
    token = app.config.get('HF_API_TOKEN') or os.environ.get('HF_API_TOKEN')
    model_id = app.config.get('HF_MODEL') or 'HuggingFaceH4/zephyr-7b-beta'
    api_url = (app.config.get('HF_API_URL') or 'https://api-inference.huggingface.co/models').rstrip('/')
    return token, model_id, api_url


def _hf_generate(prompt: str, max_tokens: int = 400, temperature: float = 0.4) -> str:
    app = current_app
    token, model_id, api_url = _get_hf_settings(app)
    if not token:
        raise AISuggestionError('HF_API_TOKEN is not configured.')
    ...
    try:
        ...
        raise AISuggestionError(f'HuggingFace request failed: {exc}') from exc

    if response.status_code >= 400:
        raise AISuggestionError(f'HuggingFace API error: {response.status_code} {response.text}')
    ...
    raise AISuggestionError('HuggingFace response was empty.')


def _openrouter_chat(prompt: str, max_tokens: int = 400, temperature: float = 0.4) -> str:
    app = current_app
    token = app.config.get('OPENROUTER_API_KEY') or os.environ.get('OPENROUTER_API_KEY')
    model_id = app.config.get('OPENROUTER_MODEL') or 'qwen/qwen3-coder:free'
    if not token:
        raise AISuggestionError('OPENROUTER_API_KEY is not configured.')
    ...
    try:
        ...
        raise AISuggestionError(f'OpenRouter request failed: {exc}') from exc

    if response.status_code >= 400:
        raise AISuggestionError(f'OpenRouter API error: {response.status_code} {response.text}')
    ...
    try:
        ...
        raise AISuggestionError('Unexpected response format from OpenRouter.') from exc



def _current_provider(app) -> str:
    provider = (app.config.get('AI_PROVIDER') or 'openrouter').strip().lower()
    if provider in ('', 'auto', 'default'):
        if app.config.get('OPENROUTER_API_KEY'):
            return 'openrouter'
        if app.config.get('OPENAI_API_KEY'):
            return 'openai'
        if app.config.get('HF_API_TOKEN'):
            return 'huggingface'
        return 'none'
    return provider
...
def generate_sat_suggestion(field: str, submission_context: Dict[str, Any]) -> str:
    ...
    if provider == 'openrouter':
        return _generate_with_openrouter(field_key, submission_context)
    if provider == 'openai':
        return _generate_with_openai(field_key, submission_context)
    if provider == 'huggingface':
        return _generate_with_hf(field_key, submission_context)
    if provider == 'none':
...
def _generate_with_openrouter(field: str, submission_context: Dict[str, Any]) -> str:
    ...
    return _openrouter_chat(prompt, max_tokens=200, temperature=0.3)

def _generate_with_hf(field: str, submission_context: Dict[str, Any]) -> str:
    audience_hint = submission_context.get('audience') or 'project stakeholders'
    prompt_sections = [
        'You are assisting with drafting a System Acceptance Testing (SAT) report.',
        'Use crisp professional language suitable for engineering documentation.',
    ...
    prompt_sections.append(f'Craft the text addressing {audience_hint}.')
    prompt = "\n".join(prompt_sections)
    return _hf_generate(prompt, max_tokens=200, temperature=0.35)
...
def _generate_contextual_response(prompt: str, context: Dict[str, Any], max_tokens: int) -> str:
    ...
    if provider == 'openrouter':
        ...
        return _openrouter_chat(full_prompt, max_tokens=max_tokens, temperature=0.6)
    ...
    if provider == 'huggingface':
        ...
        return _hf_generate(full_prompt, max_tokens=max_tokens, temperature=0.6)
...
def _analyze_intent_with_ai(message: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if provider == 'openrouter':
        prompt = (
            "Analyze the user's message and respond with a JSON object containing the keys "
            "intent, confidence, entities, urgency, and sentiment."
        )
        payload = f"{prompt}\n\nMessage: {message}\nContext: {json.dumps(context)}"
        try:
            text = _openrouter_chat(payload, max_tokens=220, temperature=0.3)
            data = _parse_json_from_text(text)
            if isinstance(data, dict):
                return {
                    'intent': data.get('intent', 'general'),
                    'confidence': float(data.get('confidence', 0.6)) if data.get('confidence') is not None else 0.6,
    ...
    if provider == 'huggingface':
        ...
        payload = f"{prompt}\n\nMessage: {message}\nContext: {json.dumps(context)}"
        try:
            text = _hf_generate(payload, max_tokens=220, temperature=0.3)
            data = _parse_json_from_text(text)
            if isinstance(data, dict):
                return {
                    'intent': data.get('intent', 'general'),
...
def _generate_ai_suggestions(context: Dict[str, Any], user_history: List[Dict[str, Any]]) -> List[str]:
    ...
    if provider == 'openrouter':
        prompt = (
            "Based on the user's context and history, suggest 4-5 practical next actions. "
        ...
        try:
            text = _openrouter_chat(f"{prompt}\n\n{user_payload}", max_tokens=200, temperature=0.45)
